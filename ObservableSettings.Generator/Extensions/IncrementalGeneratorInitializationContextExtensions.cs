...
namespace ObservableSettings.Generator.Extensions;
...
internal static class IncrementalGeneratorInitializationContextExtensions
{
    ...
    public static IncrementalValuesProvider<T> ForAttributeWithMetadataNameAndOptions<T>(
        ...
        Func<Helpers.GeneratorAttributeSyntaxContextWithOptions, CancellationToken, T> transform)
    {
        ...
        return syntaxContext.Combine(configOptions).Select((input, token) => transform(new Helpers.GeneratorAttributeSyntaxContextWithOptions(input.Left, input.Right), token));
    }
}
